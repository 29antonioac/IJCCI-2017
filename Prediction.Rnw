...
\abstract{
In this study we consider the problem of predict the winner of a \emph{StarCraft}
match in an early stage of it.
This study is done with supervised learning, using a large dataset to get
useful information. We use \emph{Spark} and \emph{MLlib} for this purpose,
due to the data dimension. We trained some algorithms and restrict matches to
