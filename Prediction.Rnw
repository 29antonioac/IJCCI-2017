...
This study is done with supervised learning, % since... or because... 
using a large dataset to get
useful information. We use \emph{Spark} and \emph{MLlib} for this
purpose, % since ... or %because... or after doing... - JJ
due to the data dimension. % You have to lay out the challenges of the
                           % problem beforehand. In this problem we
                           % have this and that. We are tackling this
                           % challenge with this, that challenge with
                           % that. 
We trained some algorithms % Use precise language. How many are some?
                           % Are they all? How did you choose them?
                           % Are the algorithms related to the
                           % challenges you should have talked about
                           % before? - JJ
and restrict matches to
some certain time instant, trying to get useful predictions with only a fraction
of the match. % DRY, don't repeat yourself. You said that before - JJ
...
in every moment of the match. % This is the motivation. With my
