...
\noindentReal Time Strategy or \emph{RTS} games are a very suitable kind of videogames
...
In Figure~\ref{fig:databaseSelection} we cab see the entity-relationship diagram
...
is different to another ones presented in Section~\ref{sec:introduction}. It
...
\begin{itemize}
  \item replay: This table contains data about each match.
  \begin{itemize}
